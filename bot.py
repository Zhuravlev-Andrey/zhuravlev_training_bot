...
Флоу:
  /start → проверка наличия тренировок → выбор/создание → выполнение
  /new   → создание новой тренировки
...
def weight_keyboard(available: list[float], current: float,
                    set_num: int, target_reps: int) -> InlineKeyboardMarkup:
    """Показывает ±4 веса от рекомендуемого + кнопку «Все веса»."""
    try:
        idx = available.index(current)
    except ValueError:
        idx = 0
    ...
    for w in subset:
        row.append(InlineKeyboardButton(
            f"{'→ ' if w == current else ''}{w}",
            callback_data=f"w_{w}_{set_num}_{target_reps}"
        ))
        if len(row) == 3:
            keyboard.append(row)
            row = []
...
def all_weights_keyboard(available: list[float],
    ...
    for w in available:
        row.append(InlineKeyboardButton(
            str(w), callback_data=f"w_{w}_{set_num}_{target_reps}"))
        if len(row) == 3:
            keyboard.append(row)
            row = []
    if row:
...
def get_set_params(ex: asyncpg.Record, set_overrides: dict, set_num: int) -> tuple[int, int, float]:
    """
    Возвращает (reps, rest_s, weight_pct) для подхода.
    Сначала берёт переопределение из set_template, иначе дефолт из exercise_template.
    """
...
async def crt_ex_equip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        "Сколько подходов? (по умолчанию 4)\n"
        "Введите число или отправьте пустое сообщение для значения по умолчанию:"
    )
    return CRT_EX_SETS
...
async def crt_ex_sets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Сколько повторений в каждом подходе? (по умолчанию 10)\n"
...
async def crt_ex_reps(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Время отдыха между подходами в секундах? (по умолчанию 90)\n"
        "Например: 60, 90, 120"
    )
    return CRT_EX_REST
...
async def crt_ex_rest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Например: «держать спину прямо» или /skip чтобы пропустить."
    )
...
async def crt_ex_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("➕ Добавить ещё упражнение", callback_data="ex_more"),
            InlineKeyboardButton("✅ Завершить создание", callback_data="ex_done"),
        ]])
    )
    return CRT_EX_MORE
...
async def start_exercise(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    last_session = await db.get_last_session_for_template(
        user_id, context.user_data["template"]["id"])
    last_str = ""
    if last_session:
        dt = last_session["started_at"].astimezone(TZ).strftime("%d.%m.%Y")
        last_str = f"\n📅 Последнее: {dt}"
    ...
    await send(update, context,
               ...
               f"Начинаем подход 1 из {ex['default_sets']}")
...
async def ask_weight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recommended = get_closest_weight(available, current * weight_pct) if available else current
    ...
    if not available:
        # Нет тренажёра — просим ввести вес текстом
        await send(update, context,
...
async def weight_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("wall_"):
        _, set_num, target_reps = query.data.split("_")
        available = context.user_data["available"]
        await query.edit_message_reply_markup(
            reply_markup=all_weights_keyboard(available, int(set_num), int(target_reps)))
        return WRK_WEIGHT
...
async def weight_text_entered(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик ввода веса текстом (когда нет тренажёра)."""
    try:
...
async def confirm_weight_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "wchange_yes":
        direction = "увеличен" if new_weight > current else "уменьшен"
        await query.edit_message_text(
            f"✅ Вес будет <b>{direction}</b> на следующей тренировке: <b>{new_weight} кг</b>",
            parse_mode="HTML")
    else:
        await query.edit_message_text(
...
async def export_do(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fmt == "json":
        buf = io.BytesIO(json.dumps(data, ensure_ascii=False,
                                    indent=2, default=str).encode())
        buf.name = f"training_export_{user_id}.json"
        await query.message.reply_document(document=buf, filename=buf.name)
    else:
        buf = io.StringIO()
        writer = csv.writer(buf)
        writer.writerow(["session_date", "template", "exercise",
                         "set", "weight_kg", "reps", "new_max_kg"])
        for sess in data.get("workout_session", []):
            for s in (sess.get("sets") or []):
                writer.writerow([
                    sess.get("started_at", "")[:10],
        ...
        csv_bytes.name = f"training_export_{user_id}.csv"
        await query.message.reply_document(document=csv_bytes, filename=csv_bytes.name)
