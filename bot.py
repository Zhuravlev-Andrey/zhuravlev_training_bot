...
def update_max_weight(current, available, set3_reps, set4_reps, set4_weight, perfect_technique):
    if set3_reps < 10:
        try:
            ...
            return current
    elif set4_reps == 10 and set4_weight == current and perfect_technique:
        try:
            idx = available.index(current)
...
async def select_training(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "❌ Отмена":
        await send_message(update, context, "Действие отменено.", reply_markup=ReplyKeyboardMarkup.remove())
...
async def start_exercise(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    default_40 = available[0]
    target_70 = current_weight * 0.7
    default_70 = get_closest_weight(available, target_70)
    ...
    await send_message(update, context,
        ...
        f"Теперь **подход 1** (цель: 10 повторений)"
    )
...
async def weight_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cancel_workout":
        # Сохраняем прогресс перед отменой
...
async def reps_entered(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if set_num < 4:
        next_set = set_num + 1
        target_reps_next = 5 if next_set == 2 else 10
...
async def finish_workout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    save_workout_log(context.user_data["workout_log"])
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await save_current_progress(context)
    await send_message(update, context, "❌ Действие отменено. Прогресс сохранён.", reply_markup=ReplyKeyboardMarkup.remove())
    context.user_data.clear()
    return ConversationHandler.END
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            SELECTING_TRAINING: [MessageHandler(filters.TEXT & ~filters.COMMAND, select_training)],
            SELECTING_WEIGHT: [CallbackQueryHandler(weight_selected)],
            ENTERING_REPS: [CallbackQueryHandler(reps_entered)],
        },
        fallbacks=[CommandHandler("cancel", cancel)]
    )
    
    app.add_handler(conv_handler)
    app.add_handler(CallbackQueryHandler(technique_answer, pattern="tech_"))
    app.add_handler(CallbackQueryHandler(weight_selected, pattern="cancel_workout"))
